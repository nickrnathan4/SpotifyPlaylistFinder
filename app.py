...
def out_to_csv(fout, dict_list):
    if dict_list:
        with open(fout, 'w') as csv_file:
            writer = csv.DictWriter(csv_file, dict_list[0].keys())
            writer.writeheader()
            writer.writerows(dict_list)


def is_valid_owner(playlist):
    if 'href' in playlist.keys():
        owner = playlist['href'].split('/')[5]
        if owner.isdigit():
            return False
...
def search_playlists(client, url, search_term=''):
    ...
    if resp:
        playlists.extend(list(map(lambda p: {'href': p['href'], 'search_term': search_term}, resp['playlists']['items'])))
...
def fetch_playlists(client, keywords=True, categories=False):
    playlists = []
    if keywords:
        playlists.extend(_fetch_playlists(client, query_set=KEYWORDS, type='keyword', page_level=3))
    if categories:
        playlists.extend(_fetch_playlists(client, query_set=TARGETED_CATEGORIES, type='categories', page_level=5))
...
def parse_line(line_in):
    ...
    return {'href': vals[0], 'search_term': vals[1]}
